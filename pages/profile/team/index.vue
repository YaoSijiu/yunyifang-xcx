					...
					<view class="work-grid">
						<view v-if="teamWorks.total == 0" class="work-item create-card-wrapper"
							...
							<view class="create-card-inner"
								@click="navigateTo('/pages/library/works/create?workType=1')">
				...
				<view v-if="currentTab === 2" class="collections-section list-mode">
					<view class="work-grid">
						<view v-if="teamCollections.total == 0" class="work-item create-card-wrapper"
							:class="{ 'single-col': isSingleColumn }">
							<view class="create-card-inner"
								@click="navigateTo('/pages/library/collection/create?workType=1')">
								<view class="cc-header">
									<view class="cc-left">
	...
	export default {
		...
		onShareAppMessage(res) {
			if (res.from === 'button') {
				...
				if ((type === 'work' || (this.currentWorkItem && this.showWorkPopup)) && this.currentWorkItem) {
					...
					return {
						...
						path: `/pages/works-detail/detail?id=${work.id}&from=team`,
					...
					return {
						...
						path: `/pages/library/collection/detail?id=${collection.id}&title=${collection.name}&from=team`,
			...
			return {
				...
				path: `pages/profile/team/agree?invitedId=${invitedId}`,
		...
		methods: {
			...
			async navigateTo(url) {
				...
				const checkMap = {
					'/pages/library/works/create': { role: TEAM_OPERATE_ROLES.MANAGE_WORKS, operateType: 1, type: 0 },
					'/pages/library/collection/create': { role: TEAM_OPERATE_ROLES.MANAGE_COLLECTIONS, operateType: 1, type: 1 }
				};
				if (checkMap[path]) {
					try {
			...
			goToWorkDetail(item) {
				uni.navigateTo({ url: `/pages/works-detail/detail?id=${item.id}&from=team` });
			},
			goToCollectionDetail(item) {
				uni.navigateTo({ url: `/pages/library/collection/detail?id=${item.id}&from=team` });
			},
			...
			async handleEditWork() {
				...
				if (!ok) return;
				uni.navigateTo({ url: `/pages/library/works/create?id=${this.currentWorkItem.id}&workType=1` });
			},
			...
			async handleEditCollection() {
				...
				if (!ok) return;
				uni.navigateTo({ url: `/pages/library/collection/create?id=${this.currentCollectionItem.id}&workType=1` });
			},
			...
			async getInviteCode(isTriggeredByUser = false) {
				...
				try {
					const path = `/pages/profile/team/agree?invitedId=${this.userInfo.id}`;
