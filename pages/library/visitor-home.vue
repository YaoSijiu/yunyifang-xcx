...
<script>
	...
	export default {
			...
			if (res.from === 'button') {
				...
				if (this.currentTab === 0 && !this.currentWorkItem && !this.currentCollectionItem) {
					...
					return {
						...
						path: `/pages/library/visitor-home?userId=${this.userId}&isTeamMode=${this.isTeamMode}`,
						imageUrl: bgImg
					};
				}
				// 作品分享
				...
				if (this.currentCollectionItem) {
					...
					return {
						...
						path: `/pages/library/collection/detail?id=${collection.id}&title=${collection.name || collection.title}&isTeamMode=${this.isTeamMode}&fromShare=1`,
		...
		methods: {
			...
			goToWorkDetail(item) {
				uni.navigateTo({
					url: `/pages/works-detail/detail?id=${item.id}&isTeamMode=${this.isTeamMode}&fromShare=1`
				});
			},

			goToWorkListDetail(item) {
				uni.navigateTo({
					url: `/pages/library/collection/detail?id=${item.id}&title=${item.name || item.title}&isTeamMode=${this.isTeamMode}&fromShare=1`
				});
			},
			...
			showLoginModal() {
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							const redirectUrl = `/pages/library/visitor-home?userId=${this.userId}&isTeamMode=${this.isTeamMode}`;
