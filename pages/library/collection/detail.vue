	...
	export default {
		...
		onShareAppMessage(res) {
			if (res.from === 'button' && this.currentWorkItem) {
				...
				return {
					...
					path: `/pages/works-detail/detail?id=${work.id}&isTeamMode=${this.isTeamMode}&fromShare=1`,
			...
			return {
				...
				path: `/pages/library/collection/detail?id=${this.collectId}&title=${encodeURIComponent(this.getCollectionShareTitle())}&isTeamMode=${this.isTeamMode}&fromShare=1`,
		...
		methods: {
			...
			goToVisitorHome() {
				...
				uni.navigateTo({
					url: `/pages/library/visitor-home?userId=${this.userInfo.id}&isTeamMode=${this.isTeamMode}`
				});
			},
			...
			goToWorkDetail(item) {
				...
				uni.navigateTo({
					url: `/pages/works-detail/detail?id=${item.id}${shareParam}`
				});
			},
			...
			async handleEditCollection() {
				...
				uni.navigateTo({
					url: `/pages/library/collection/create?id=${this.collectId}&title=${encodeURIComponent(this.title || '')}&workType=${this.isTeamMode ? 1 : 0}`
				});
			},
			...
			async handleEditWork() {
				...
				uni.navigateTo({
					url: `/pages/library/works/create?id=${this.currentWorkItem.id}&title=${this.currentWorkItem.title}&workType=${this.isTeamMode ? 1 : 0}`
				});
			},
