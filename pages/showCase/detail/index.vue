...
export default {
	...
	methods: {
		...
		goToSellerHome() {
			...
			uni.navigateTo({
				url: `/pages/library/visitor-home?userId=${encodeURIComponent(this.followUserId)}&isTeamMode=false`
			});
		},
		...
		handleHire() {
			...
			uni.navigateTo({
				url: `/pages/task/index?publishType=invite&inviteeWxUserId=${encodeURIComponent(this.followUserId)}&source=showcase&showcaseId=${encodeURIComponent(this.showcaseId)}`
			});
		},
		openPayPopup() {
			this.handleBuyClick();
	...
	onShareAppMessage() {
		return {
			title: this.detail.showcaseTitle || '橱窗详情',
			path: `/pages/showCase/detail/index?id=${this.showcaseId}&fromShare=1`,
