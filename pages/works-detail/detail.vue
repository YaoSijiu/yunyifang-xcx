...
<script>
	...
	export default {
		...
		methods: {
			...
			goToAbout() {
				uni.switchTab({
					url: `/pages/library/visitor-home/${this.productInfo}`
				});
			},
			// 点击右侧分享
			handleShare() {
			...
			previewImage(current, index) {
				...
				uni.navigateTo({
					url: `/pages/works-detail/image-preview?urls=${encodeURIComponent(JSON.stringify(urls))}&index=${index}&productId=${this.productId}&isOwner=${this.isOwner}`
				});
			},
			// 文档预览
		...
		onShareAppMessage() {
			return {
				...
				path: `/pages/works-detail/detail?id=${this.productId}&isTeamMode=${this.isTeamMode}&fromShare=1`,
