...
export default {
	...
	onShareAppMessage() {
		return {
			...
			path: `/pages/task/detail/index?channelId=${this.channelId}&id=${this.channelId}`,
	...
	methods: {
		...
		promptLogin() {
			...
			if (this.channelId) {
				uni.setStorageSync('login_redirect', `/pages/task/detail/index?channelId=${this.channelId}&id=${this.channelId}`);
			}
			uni.showModal({
				title: '提示',
				content: '登录后可使用该功能',
		...
		goPublisherHome() {
			...
			uni.navigateTo({
				url: `/pages/library/visitor-home?userId=${encodeURIComponent(this.ownerUserId)}`
			});
		},
